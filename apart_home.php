...
if($area_sub_name!=''){
  $sql = "
        select
          meme.area_main_name,
        	...
        	(
      		...
      		WHERE a.area_main_name = meme.area_main_name
      		AND a.dong = meme.dong
      		AND a.apart_name = meme.apart_name
      		AND ROUND(b.exclsSpc,0) = meme.size
      		LIMIT 1
    		  ) AS supply_size,
        	meme.dong,
        	meme.max_price,
        ...
        from molit_max_min_all_group meme LEFT join molit_max_min_rent_all_group rent
...
<h2><?php if ($area_main_name!="지역선택") {echo $area_main_name;} ?> <?php if ($area_main_name!="지역선택") {echo $area_sub_name;} ?> <?php echo $dong; ?> 실거래 매매 리스트, 검색조건당 최대 100개 조회</h2>
...
<table>
    ...
    <tr>
      <?php if($isMobile == "Y") { ?>
        ...
        <th style="font-size: 18px; width:19%;"><b>최고가격<br>대비하락<br>(전세가율)</b></th>
        <th style="font-size: 18px; width:17%;"><b>최근거래<br>(최근전세)</b></th>
        <th style="font-size: 18px; width:17%;"><b>최고가격<br>(최고전세)</b></th>
      <?php }else { ?>
        <th style="font-size: 20px; width:33%;"><b>아파트</b></th>
    ...
    <tbody>
      ...
      <tr>
          <td style="font-size: 20px; width:32%;"><a href='./apart.php?area_main_name=<?=$row[area_main_name]?>&apart_name=<?=$row[apart_name]?>&size=<?=$row[size]?>&dong=<?=$row[dong]?>&all_area=N'><b><?=$row['apart_name']?></b><br><?=$row['area_name']?> <?=$row['dong']?></td>
          <!--<td style="font-size: 20px;"><b><?=$row['size']?>㎡</b></td>-->
          <td style="font-size: 20px; width:15%;"><b><?=$row['size']?>㎡</b><br><?=$row['supply_size']?></td>
          <td style="background-color:rgba(255, 0, 0, 0.3); font-size: 20px; width:19%;"><b><?=$row['price_last']?>억<br><?=$row['percent']?>%하락</b><br>(<?=$row['rent_rate']?>%)</td>
          <td style="font-size: 20px; width:17%;"><b><?=$row['last_price']?>억</b><br><?=$row['last_price_date']?><br>(<?=$row['rent_last_price']?>억)</td>
          <td style="font-size: 20px; width:17%;"><b><?=$row['max_price']?>억</b><br><?=$row['max_price_date']?><br>(<?=$row['rent_max_price']?>억)</td>
          <?php if($isMobile == "N") { ?>
...
if ( $main == '1'){
  $main_text_rent = "AND cast(rent.size as DECIMAL(10,5)) > '49.99999'";
}elseif ($main == ''){
  $main = "1";
  $main_text_rent = "AND cast(rent.size as DECIMAL(10,5)) > '49.99999'";
}else{
...
$rs_count_rent = mysqli_num_rows($rs_rent);
...
<h2><?php if ($area_main_name!="지역선택") {echo $area_main_name;} ?> <?php if ($area_main_name!="지역선택") {echo $area_sub_name;} ?> <?php echo $dong; ?> 실거래 전세 리스트, 검색조건당 최대 100개 조회</h2>
<table>
    <thead>
    <tr>
      <?php if($isMobile == "Y") { ?>
        <th style="font-size: 20px; background: #809EAD; width:32%;"><b>아파트</b></th>
        <th style="font-size: 18px; background: #809EAD; width:15%;"><b>전용면적</b><br><b>공급면적</b><br>(공급평형)</th>
        <th style="font-size: 18px; background: #809EAD; width:19%;"><b>최고가격<br>대비하락</b><br>(전세가율)</th>
        <th style="font-size: 18px; background: #809EAD; width:17%;"><b>최근거래</b><br>(최근매매)</th>
        <th style="font-size: 18px; background: #809EAD; width:17%;"><b>최고가격</b><br>(최고매매)</th>
      <?php }else { ?>
        <th style="font-size: 20px; background: #809EAD; width:35%;"><b>아파트</b></th>
        <th style="font-size: 20px; background: #809EAD; width:13%;"><b>전용면적</b><br><b>공급면적</b><br>(공급평형)</th>
        <th style="font-size: 20px; background: #809EAD; width:13%;"><b>최고가격<br>대비하락</b><br>(전세가율)</th>
        <th style="font-size: 20px; background: #809EAD; width:14%;"><b>최근거래</b><br>(최근매매)</th>
        <th style="font-size: 20px; background: #809EAD; width:14%;"><b>최고가격</b><br>(최고매매)</th>
        <th style="font-size: 20px; background: #809EAD; width:14%;"><b>최저가격</b><br>(최저매매)</th>
      <?php } ?>
    </tr>
    </thead>
    <tbody>
      <?php $add_count_rent = 0; foreach ($rows_rent as $row_rent) { if($add_count_rent!=0 && fmod($add_count_rent, 15)==0 && $isMobile == "Y"){echo '<tr><td colspan="6"><script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-2265060002718871" crossorigin="anonymous"></script> <ins class="adsbygoogle" style="display:block" data-ad-format="fluid" data-ad-layout-key="-fb+5w+4e-db+86" data-ad-client="ca-pub-2265060002718871" data-ad-slot="3474043280"></ins> <script> (adsbygoogle = window.adsbygoogle || []).push({}); </script></td></tr>';} $add_count_rent = $add_count_rent + 1; ?>
      <tr>
          <td style="font-size: 20px; width:32%;"><a href='./apart_rent.php?area_main_name=<?=$row_rent[area_main_name]?>&apart_name=<?=$row_rent[apart_name]?>&size=<?=$row_rent[size]?>&dong=<?=$row_rent[dong]?>&all_area=N'><b><?=$row_rent['apart_name']?></b><br><?=$row_rent['area_name']?> <?=$row_rent['dong']?></td>
          <td style="font-size: 20px; width:15%;"><b><?=$row_rent['size']?>㎡</b><br><?=$row_rent['supply_size']?></td>
          <td style="background-color:rgba(255, 0, 0, 0.3); font-size: 20px; width:19%;"><b><?=$row_rent['price_last']?>억<br><?=$row_rent['percent']?>%하락</b><br>(<?=$row_rent['rent_rate']?>%)</td>
          <td style="font-size: 20px; width:17%;"><b><?=$row_rent['last_price']?>억</b><br><?=$row_rent['last_price_date']?><br>(<?=$row_rent['meme_last_price']?>억)</td>
          <td style="font-size: 20px; width:17%;"><b><?=$row_rent['max_price']?>억</b><br><?=$row_rent['max_price_date']?><br>(<?=$row_rent['meme_max_price']?>억)</td>
          <?php if($isMobile == "N") { ?>
          <td style="font-size: 20px;"><b><?=$row_rent['min_price']?>억</b><br><?=$row_rent['min_price_date']?><br>(<?=$row_rent['meme_min_price']?>억)</td>
          <?php }?>
      </tr>
      <?php } ?>
    </tbody>
