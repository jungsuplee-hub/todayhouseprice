...
if($size1=="true"){
  $size1_text = "or cast(today.size as decimal(10,2)) <= 40";
}
if($size2=="true"){
  $size2_text = "or (cast(today.size as decimal(10,2)) > 40 and cast(today.size as decimal(10,2)) <= 60)";
}
if($size3=="true"){
  $size3_text = "or (cast(today.size as decimal(10,2)) > 60 and cast(today.size as decimal(10,2)) <= 85)";
}
if($size4=="true"){
  $size4_text = "or cast(today.size as decimal(10,2)) > 85";
}
...
$sql_rent = "
      ...
      AND ROUND(CAST(today.size as DECIMAL(10,2))) = meme.size
      where $area_main_name_text
      and today.insert_date = '$insert_date'
...
$sql_favorite = "
    ...
    and (today.area_main_name, today.doing, today.apart_name, ROUND(CAST(today.size as DECIMAL(10,2)))) in (select area_main_name, dong, apart_name, size from molit_favorite where userid = '$userid')
    and today.status is not null
    AND !(today.last_price = '0' AND today.max_price != '0')
    ORDER BY ABS(ROUND((CAST(today.price as DECIMAL(10,5)) - CAST(today.last_price as DECIMAL(10,5))),2)) desc
    limit 100;
...
$sql_favorite_rent = "
      ...
      where today.insert_date = '$insert_date'
      and today.month_price = '0'
      and (today.area_main_name, today.dong, today.apart_name, ROUND(CAST(today.size as DECIMAL(10,2)))) in (select area_main_name, dong, apart_name, size from molit_favorite where userid = '$userid')
      and today.status is not null
      ORDER BY ABS(ROUND((CAST(today.rent_price as DECIMAL(10,5)) - CAST(today.last_price as DECIMAL(10,5))),2)) desc
      limit 100;
      ";
...
<table>
    ...
    <tr>
      ...
      <th style="background: #809EAD; width:16%; padding-top:15px; padding-bottom:10px;"><center><a style="font-size: 20px; color:white; text-decoration: none;" href='./apart_trend.php'>트렌트</a></center></th>
      <th style="background: #809EAD; width:16%; padding-top:15px; padding-bottom:10px;"><center><a style="font-size: 20px; color:white; text-decoration: none;" href='./list.php'>오늘집값 랭킹</a></center></th>
