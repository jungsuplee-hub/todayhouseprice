...
<?php if($isMobile=='Y'){ ?>
    <input id="search" type="search" size="30" style="font-size:25px; border-radius: 15px; border-color:#73685d; outline: none; padding-bottom: 5px; padding-top: 8px; padding-left: 20px;" placeholder="아파트명,주소 검색,  최대 10개" onkeyup="showResult(this.value)" autofocus autocomplete="off">
<?php }else{ ?>
    <input id="search" type="search" size="40" style="font-size:25px; border-radius: 15px; border-color:#73685d; outline: none; padding-bottom: 5px; padding-top: 8px; padding-left: 20px;" placeholder="아파트명,주소 검색,  최대 10개" onkeyup="showResult(this.value)" autofocus autocomplete="off">
<?php } ?>
</form>
</center>
<br>
...
<select style="width:200px;font-size:25px;" name="day" id="day" onchange="apart_day_list(this)">
...
<span style="font-size:25px;"><b>신규 등록된 </b></span>

<select style="width:170px;font-size:25px;" name="type" id="type" onchange="apart_type(this)">
...
<span style="font-size:25px;"><b>리스트</b></span>
<br>


<span style="font-size:25px;"></b></span><select style="width:220px;font-size:25px;" name="main" id="main" onchange="apart_list(this)">
<?php
...
<span style="font-size:25px;"><b>시군구 : </b></span><select style="width:220px;font-size:25px;" name="sub" id="sub" onchange="apart_sub_list(this)">
...
<table>
    ...
    <tr>
        <?php if($isMobile=='Y'){ ?>
            <th style="font-size: 25px; width:37%;"><b>아파트명<br>거래일자</b></th>
            <th style="font-size: 25px; width:14%;"><b>전용면적</b><br>층<br>거래유형</b></th>
            <th style="font-size: 25px; width:18%;"><b>신규가격<br>가격변동<br>변동률<br>최근가격</b></th>
            <th style="font-size: 25px; width:18%;"><b>최고대비<br>하락/상승률<br>최고가격</b></th>
        <?php }else{ ?>
      ...
      <?php $add_count = 0; foreach ($rows as $row) { if($add_count!=0 && fmod($add_count, 15)==0){echo '<tr><td colspan="6"><script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-2265060002718871" crossorigin="anonymous"></script> <ins class="adsbygoogle" style="display:block" data-ad-format="fluid" data-ad-layout-key="-fb+5w+4e-db+86" data-ad-client="ca-pub-2265060002718871" data-ad-slot="3474043280"></ins> <script> (adsbygoogle = window.adsbygoogle || []).push({}); </script></td></tr>';} $add_count = $add_count + 1; ?>
        <?php if($isMobile=='Y'){ ?>
            <?php
              $bg_color = "";
              if ( $row['STATUS']== '신고가') {
                $bg_color = "0, 255, 0, 0.7";
              } elseif ( $row['STATUS']== '상승') {
                $bg_color = "0, 255, 0, 0.4";
              } elseif ( $row['STATUS']== '동일') {
                $bg_color = "255, 255, 0, 0.5";
              } elseif ( $row['STATUS']== '하락') {
                $bg_color = "255, 0, 0, 0.4";
              } elseif ( $row['STATUS']== '신저가') {
                $bg_color = "255, 0, 0, 0.7";
              }else{
                $bg_color = "";
              }
            ?>
            <tr></tr>
            <tr>
                <td rowspan="2" style="font-size: 25px;"><a href='./apart.php?area_main_name=<?=$row[area_main_name]?>&apart_name=<?=$row[apart_name]?>&size=<?=$row[size]?>&dong=<?=$row[doing]?>&all_area=N'><b><span style="font-size: 27px;"><?=$row['apart_name']?></span></b><br><?=$row['yearmonthday']?></b><br><?=$row['area_main_name']?> <?=$row['area_name']?> <?=$row['doing']?></td>
                <td rowspan="2" style="font-size: 25px;"><b><?=$row['size']?>㎡</b><br><?=$row['stair']?>층<br><?=$row['TYPE']?></td>
                
                <td style="background-color:rgba(<?=$bg_color?>); font-size: 25px;"><b><?=$row[price]?>억</b><br><?=$row[price_last]?>억<br><?=$row[percent]?>% <?=$row[STATUS]?></td>
                <td style="font-size: 25px;"><b><?=$row['price_max']?>억<br><?php if($row['max_percent']>=30){ echo "<span style='color:red;'>최고가 대비<br>"; echo $row['max_percent']; echo "%하락</span>";} elseif($row['max_percent']<0){ echo "<span style='color:green;'>최고가 대비<br>"; echo abs($row['max_percent']); echo "%상승</span>";} else{ echo "최고가 대비<br>"; echo $row['max_percent']; echo "%하락";} ?></b></td>
            </tr>
            <tr>
                <td style="background-color:rgba(<?=$bg_color?>); font-size: 25px;"><b><?=$row['last_price']?>억</b><br><?=$row['last_price_date']?></td>
                <td style="font-size: 25px;"><b><?=$row['max_price']?>억</b><br><?=$row['max_price_date']?></td>
            </tr>
