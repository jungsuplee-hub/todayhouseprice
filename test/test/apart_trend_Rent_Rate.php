...
IFNULL(CAST(MAX(case when area = '$area_main_name_short' then cnt end) as DECIMAL(10,2)),0) as cnt1
from RentRate
group by yyyymm
order by yyyymm
";
...
$sql_min_max = "
select min(CAST(cnt as DECIMAL(10,0)))-5 as temp_min, max(CAST(cnt as DECIMAL(10,0)))+5 as temp_max
FROM RentRate
...
<br>
<span style="font-size:20px;">한국 부동산원 자료를 바탕으로 만들어진 데이터 입니다. <a href='https://kosis.kr/statHtml/statHtml.do?orgId=408&tblId=DT_30404_N0006_R1'>[출처링크]</a></span>
<br><br>
...
function apart_list(e) {
  <?php echo "window.location.replace('./apart_trend_Rent_Rate.php?'+'area_main_name='+document.getElementById('main').value);"?>
}
</script>

<span style="font-size:30px;">아파트 매매가격 대비 전세가격 비율 그래프</span>
<br><br>
...
<center><div id="lineChart_div" style="width: 100%; height: 500px"></div></center>
