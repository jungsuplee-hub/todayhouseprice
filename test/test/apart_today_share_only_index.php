...
<h1>전국 아파트 거래량 그래프</h1>
<span style="font-size:20px;">국토부 제공 실거래 자료를 바탕으로 하기 때문에 참고 용도로만 사용하시기 바랍니다.<br></span>
...
<br>
<span style="font-size:30px;">매매 거래량 그래프</span>
<br>
<center><div id="memeChart_div" style="width: 100%; height: 300px"></div></center>
<br>

<span style="font-size:30px;">전월세 거래량 그래프</span>
<br>
...
CAST(BUY_DOM_INDICES as DECIMAL(10,2)) as temp
from AptTradingMarketTrend
where 1=1
and LEVEL_NO = 0
and REGION_CD = 'A1000'
...
$sql_min_max2 = "
select
MIN(CAST(BUY_DOM_INDICES as DECIMAL(10,2))) as temp_min,
MAX(CAST(BUY_DOM_INDICES as DECIMAL(10,2))) as temp_max
from AptTradingMarketTrend
where 1=1
and LEVEL_NO = 0
and REGION_CD = 'A1000'
...
$sql2 = "
  ...
  CAST(MAX(case when REGION_CD = 'A1000' then BUY_DOM_INDICES end) as DECIMAL(10,2)) as case_BUY_A1000,
  CAST(MAX(case when REGION_CD = '11000' then BUY_DOM_INDICES end) as DECIMAL(10,2)) as case_BUY_11000,
  CAST(MAX(case when REGION_CD = 'A2000' then BUY_DOM_INDICES end) as DECIMAL(10,2)) as case_BUY_A2000,
  CAST(MAX(case when REGION_CD = 'A2001' then BUY_DOM_INDICES end) as DECIMAL(10,2)) as case_BUY_A2001,
  CAST(MAX(case when REGION_CD = 'A3000' then BUY_DOM_INDICES end) as DECIMAL(10,2)) as case_BUY_A3000,
  CAST(MAX(case when REGION_CD = 'A9000' then BUY_DOM_INDICES end) as DECIMAL(10,2)) as case_BUY_A9000
  from
  (
  select RESEARCH_DATE, REGION_CD, BUY_DOM_INDICES
  from AptTradingMarketTrend
  where 1=1
  and LEVEL_NO = 0
...
<h1>전국 아파트 매매 수급동향 그래프</h1>
<span style="font-size:20px;">국토부 제공 자료를 바탕으로 만들어진 데이터 입니다.<br>매매수급지수는 100을 기준으로, 이보다 낮을수록 집을 사려는 수요가 적음을 나타낸다.</span>
<br>
<br>
<script type="text/javascript">
    ...
    function drawChart() {
        ...
        //console.log(JSON.stringify(chart_array))
        var header = ['Date&Time(MM-DD HH:MM)', '매매수급동향'];
        var row = "";
        var rows = new Array();
        jQuery.each(chart_array, function(index, item) {
            row = [
...
<center><div id="lineChart_div2" style="width: 100%; height: 300px"></div></center>
...
<table>
    ...
    <tbody>
      ...
      <tr>
          <td style="font-size: 20px;"><b><?=$row2['RESEARCH_DATE']?></b></td>
          <td style="font-size: 20px;"><b><?=$row2['case_BUY_A1000']?></b></td>
          <td style="font-size: 20px;"><b><?=$row2['case_BUY_11000']?></b></td>
          <td style="font-size: 20px;"><b><?=$row2['case_BUY_A2000']?></b></td>
          <td style="font-size: 20px;"><b><?=$row2['case_BUY_A2001']?></b></td>
          <td style="font-size: 20px;"><b><?=$row2['case_BUY_A3000']?></b></td>
          <td style="font-size: 20px;"><b><?=$row2['case_BUY_A9000']?></b></td>
      </tr>
      <?php } ?>
    </tbody>
</table>
...
$sql_min_max2_1 = "
select
MIN(CAST(JEON_SND_INDICES as DECIMAL(10,2))) as temp_min,
MAX(CAST(JEON_SND_INDICES as DECIMAL(10,2))) as temp_max
from AptRentalMarketTrend
where 1=1
and LEVEL_NO = 0
and REGION_CD = 'A1000'
...
$result_chart2_1 = mysqli_query($Conn, $sql_chart2_1);

while ($row_chart2_1 = mysqli_fetch_assoc($result_chart2_1)) {
    $data_array2_1[] = $row_chart2_1;
}
$chart2_1 = json_encode($data_array2_1);

$result_min_max2_1 = mysqli_query($Conn, $sql_min_max2_1);
$row_min_max2_1 = mysqli_fetch_assoc($result_min_max2_1);
//여기는 그래프 관련


$rs2_1 = mysqli_query($Conn, $sql2_1);

while ( $row2_1 = mysqli_fetch_assoc($rs2_1) ) {
    $rows2_1[] = $row2_1;
}

?>

<h1>전국 아파트 전세 수급동향 그래프</h1>
<span style="font-size:20px;">국토부 제공 자료를 바탕으로 만들어진 데이터 입니다.<br>전세수급지수는 100을 기준으로, 이보다 낮을수록 집을 구하려는 수요가 적음을 나타낸다.</span>
<br>
<br>
<script type="text/javascript">
    google.charts.load('current', { packages: ['corechart', 'line'] });
    ...
    function drawChart() {
        var chart_array = <?php echo $chart2_1; ?>;
        //console.log(JSON.stringify(chart_array))
        ...
        var lineChartOptions = {
            ...
            vAxes: {
                0: {
                    viewWindow: { min: <?=$row_min_max2_1['temp_min']?>, max: <?=$row_min_max2_1['temp_max']?> }
                }
            },
            interpolateNulls : true
            //,
        ...
        var lineChart = new google.visualization.LineChart(document.getElementById('lineChart_div2_1'));
        lineChart.draw(data, lineChartOptions);
...
<center><div id="lineChart_div2_1" style="width: 100%; height: 300px"></div></center>
...
<table>
    ...
    <tbody>
      <?php foreach ($rows2_1 as $row2_1) { ?>
      <tr>
          <td style="font-size: 20px;"><b><?=$row2_1['RESEARCH_DATE']?></b></td>
          <td style="font-size: 20px;"><b><?=$row2_1['case_BUY_A1000']?></b></td>
          <td style="font-size: 20px;"><b><?=$row2_1['case_BUY_11000']?></b></td>
          <td style="font-size: 20px;"><b><?=$row2_1['case_BUY_A2000']?></b></td>
          <td style="font-size: 20px;"><b><?=$row2_1['case_BUY_A2001']?></b></td>
          <td style="font-size: 20px;"><b><?=$row2_1['case_BUY_A3000']?></b></td>
          <td style="font-size: 20px;"><b><?=$row2_1['case_BUY_A9000']?></b></td>
      </tr>
      <?php } ?>
    </tbody>
</table>
...
<script type="text/javascript">
    ...
    function drawChart() {
        ...
        var lineChartOptions = {
            title: '전국 미분양 추이',
            chartArea: {left: '7%', width: '91%', height:'73%'},
            seriesType: "bars",
            isStacked: true,
            hAxis: {
...
<script type="text/javascript">
    ...
    function drawChart() {
        ...
        var lineChartOptions = {
            title: '전국 미분양 추이',
            chartArea: {left: '7%', width: '88%', height:'73%'},
            seriesType: "bars",
