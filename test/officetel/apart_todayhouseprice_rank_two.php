...
<title>오늘집값 - 오피스텔 랭킹(최고금액대비 하락가격 Top 100)</title>
...
and CAST(max_price as DECIMAL(10,5)) > 3
and CAST(last_price as DECIMAL(10,5)) > 3
and max_price_date like '202%'
and (last_price_date like '2021%' or last_price_date like '2022%' or last_price_date like '2023%' or last_price_date like '2024%')
order by ROUND(100-(CAST(last_price as DECIMAL(10,5))/CAST(max_price as DECIMAL(10,5))*100) ,2) desc
limit 100;
      ";
...
<table>
    <thead>
    <tr>
      <th style="background: #FBE5D6; width:25%; font-size: 30px; padding-top:5px; padding-bottom:0px;"><center><a style="text-decoration: none;" href='../apart_todayhouseprice_rank_two.php'>아파트</a></center></th>
      <th style="background: #C0C0C0; width:25%; font-size: 30px; color:black; padding-top:5px; padding-bottom:0px;"><center>오피스텔</center></th>
      <th style="background: #E2F0D9; width:25%; font-size: 30px; padding-top:5px; padding-bottom:0px;"><center><a style="text-decoration: none;" href='../billa/apart_todayhouseprice_rank_two.php'>다세대주택(빌라)</a></center></th>
    </tr>
  </thead>
</table>
<table>
...
<table>
    ...
    <tr>
      ...
      <th style="background-color:rgba(0, 0, 0, 0.3); width:25%; padding-top:15px; padding-bottom:10px;"><center><a style="font-size: 20px; color:white; text-decoration: none;" href='./apart_todayhouseprice_rank_two.php'>하락률 Top 100</a></center></th>
      <th style="background-color:rgba(255, 255, 255, 0.8); width:25%; padding-top:15px; padding-bottom:10px;"><center><a style="font-size: 20px; color:black; text-decoration: none;" href='./apart_todayhouseprice_rank_three.php'>가격 Top 100</a></center></th>
...
function check1(country){
  location.href = "./apart_todayhouseprice_rank_two.php?area_main_name=<?=$area_main_name?>&size1="+document.getElementById("size1").checked+"&size2="+document.getElementById("size2").checked+"&size3="+document.getElementById("size3").checked+"&size4="+document.getElementById("size4").checked;
}
...
<h1><?=$area_main_name?> 오피스텔 최고금액 대비 마지막거래 가격 하락률 Top 100 </h1>
<span style="font-size: 20px;">*참고<br>. 거래취소 내용이 반영되지 않은 케이스가 일부 있을 수 있습니다.<br>. 직거래 가격이 포함되어 있어 조금 튀는 데이터가 있을 수 있습니다.<br>. 3억이상 가격을 기준으로 했으며, 최고가격은 2020년 이후, <br>&nbsp;&nbsp;마지막거래가격은 2021년 이후 데이터를 기준으로 하였습니다.</span>
<br><br>
...
<table>
    ...
    <tr>
        ...
        <th style="font-size: 20px; width:40%;"><b>오피스텔명<br>지역</b></th>
        <th style="font-size: 20px; width:12%;"><b>전용면적</b></th>
        <th style="font-size: 20px; width:15%;"><b>하락률<br>하락금액</b></th>
        <th style="font-size: 20px; width:13%;"><b>최고가격</b></th>
        <th style="font-size: 20px; width:13%;"><b>마지막<br>거래가격</b></th>
    ...
    <tbody>
      ...
      <tr>
          ...
          <td style="font-size: 20px;"><b><?php if($row['diff_rate']>50){ echo "<span style='color:fuchsia;'>"; }else if($row['diff_rate']>30){ echo "<span style='color:red;'>"; } ?><?=$row['diff_rate']?>%<?php if($row['diff_rate']>30){ echo "</span>"; } ?><br>-<?=$row['diff_price']?>억</b></td>
          <td style="font-size: 20px;"><b><?=$row['max_price']?>억</b><br><?=$row['max_price_date']?></td>
          <td style="font-size: 20px;"><b><?=$row['last_price']?>억</b><br><?=$row['last_price_date']?></td>
      </tr>
      <?php } ?>
    </tbody>
</table>
...
function apart_list(e) {
  <?php echo "window.location.replace('./apart_todayhouseprice_rank_two.php?'+'area_main_name='+document.getElementById('main').value+'&size1=$size1'+'&size2=$size2'+'&size3=$size3'+'&size4=$size4');"?>
}
</script>
