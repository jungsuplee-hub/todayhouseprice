...
<title>오늘집값 - 오피스텔 매매 상세 조회</title>
<meta name="description" content="오피스텔 매매, 전세 실거래가를 확인해보세요. 당일 등록된 매매/전세정보를 확인하고 본인만의 즐겨찾기를 설정 할 수 있습니다.">
<meta property="og:type" content="website">
<meta property="og:title" content="오피스텔 매매 상세검색">
<meta property="og:description" content="오피스텔 매매  정보를 상세히 검색 할 수 있습니다.">
...
$rs_row_count = mysqli_num_rows($rs);
...
if($rs_row_count==0){
  $sql = "
      ...
      FROM molit_info_all_officetel
      where area_main_name = '$area_main_name'
      and apart_name like '%$apart_name%'
      and dong = '$dong'
      $size_text
      order by date_format(concat(year,'/',month,'/',day),'%Y-%m-%d') desc
      ";
...
//여기는 그래프 관련
if($rs_row_count==0){
  $sql_chart = "SELECT STR_TO_DATE(CONCAT(YEAR,LPAD(MONTH,'2','0'),LPAD(DAY,'2','0')), '%Y%m%d') AS dt, CAST(price AS DECIMAL(10,6)) AS temp  from molit_info_all_officetel  WHERE area_main_name = '$area_main_name'  and apart_name like '%$apart_name%' and dong = '$dong'  and ROUND(CAST(size as DECIMAL(10,5))) = '$size'  ORDER BY STR_TO_DATE(CONCAT(YEAR,LPAD(MONTH,'2','0'),LPAD(DAY,'2','0')), '%Y%m%d')";
  $sql_min_max = "SELECT ROUND(count(1)/10) as cnt, ROUND(MAX(CAST(price AS DECIMAL(10,5)))+2) AS temp_max, ROUND(MIN(CAST(price AS DECIMAL(10,5)))-2) AS temp_min  from molit_info_all_officetel  WHERE area_main_name = '$area_main_name'  and apart_name like '%$apart_name%' and dong = '$dong' and ROUND(CAST(size as DECIMAL(10,5))) = '$size'";

}else{
  $sql_chart = "SELECT STR_TO_DATE(CONCAT(YEAR,LPAD(MONTH,'2','0'),LPAD(DAY,'2','0')), '%Y%m%d') AS dt, CAST(price AS DECIMAL(10,6)) AS temp  from molit_info_all_officetel  WHERE area_main_name = '$area_main_name'  and apart_name = '$apart_name' and dong = '$dong'  and ROUND(CAST(size as DECIMAL(10,5))) = '$size'  ORDER BY STR_TO_DATE(CONCAT(YEAR,LPAD(MONTH,'2','0'),LPAD(DAY,'2','0')), '%Y%m%d')";
  $sql_min_max = "SELECT ROUND(count(1)/10) as cnt, ROUND(MAX(CAST(price AS DECIMAL(10,5)))+2) AS temp_max, ROUND(MIN(CAST(price AS DECIMAL(10,5)))-2) AS temp_min  from molit_info_all_officetel  WHERE area_main_name = '$area_main_name'  and apart_name = '$apart_name' and dong = '$dong' and ROUND(CAST(size as DECIMAL(10,5))) = '$size'";
...
<table>
    <thead>
    <tr>
      <th style="background: #FBE5D6; width:25%; font-size: 30px; padding-top:5px; padding-bottom:0px;"><center><a style="text-decoration: none;" href='../apart_home.php'>아파트</a></center></th>
...
<a style="float:right;" href='./apart_rent.php?area_main_name=<?php echo $area_main_name; ?>&apart_name=<?php echo $apart_name; ?>&size=<?php echo $size; ?>&dong=<?php echo $dong; ?>&all_area=<?php echo $all_area; ?>'>해당 주택 전/월세 정보 보기</a>
</h1>
<h2>
<a target="_blank" href='https://m.land.naver.com/search/result/<?php echo str_replace(' ','%20',$dong); ?><?php echo str_replace(' ','%20',$apart_name); ?>'>네이버 부동산 바로가기</a>
<a style="float:right;" target="_blank" href='https://hogangnono.com/search?q=<?php echo str_replace(' ','%20',$dong); ?> <?php echo str_replace(' ','%20', str_replace($dong,'',$apart_name)); ?>오피스텔'>호갱노노 바로가기</a>
</h2>
