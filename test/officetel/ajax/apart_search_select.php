...
$sql="SELECT area_main_name, replace(area_sub_name,' ','') as area_sub_name, replace(dong,' ','') as dong, replace(apart_name,' ','') as apart_name, build_year FROM officetel_dong where (apart_name like '%$q%' or area_main_name like '%$q%'  or area_sub_name like '%$q%'  or dong like '%$q%') and doro is not null limit 10";
...
echo "<table>
    <thead>
    <tr>
        <th style='font-size: 20px; width:35%;'><b>오피스텔명</b></th>
...
while($row = mysqli_fetch_array($result))
  ...
  echo "<td style='font-size: 20px;'>" . $row['area_main_name'] . " ".$row['area_sub_name']." ".$row['dong']."</td>";
