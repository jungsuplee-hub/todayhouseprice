...
<?php include_once "./head.php";?>
	...
	<body>
		...
		<br>
		<center>
		<span style="font-size:30px; vertical-align: middle;"><b>회원가입</b></span>
		</center>
		<br>
		<div class="container">
				...
				<div class="jumbotron" style="padding-top: 20px;">
					<form name="join"  method="post" action="join_ok.php">
						...
						<div class="form-group">
							<input type="text" class="form-control" placeholder="LG 협력사 이메일" name="id" id="id" maxlength="40">
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<input type="email" class="form-control" placeholder="확인용이메일 (비밀번호 초기화용)" name="email" id="email" maxlength="80">
						</div>
		...
		<script>
			...
			$(function(){/*문서가 로드되면 function을 실행하라  */
				$("#id").blur(function(){/*아이디가 id인것을 찾아 포커즈를 빠져나갈때 발생하는 이벤트  */
					if($(this).val()==""){
						$("#id_check_msg").html("LG 협력사 이메일를 입력하세요.").css("color","red").attr("data-check","0");/*선택자를 .연사자추가해서 계속 사용가능  */
						$(this).focus();
					}else if ($(this).val().indexOf('@cnspartner.com') == -1){
					  $("#id_check_msg").html("LG 협력사 이메일이 아닙니다.").css("color","red").attr("data-check","0");/*선택자를 .연사자추가해서 계속 사용가능  */
						$(this).focus();
					}else{
						checkIdAjax();
					}
				...
				$.ajax({				//비동기통신방법, 객체로 보낼때{}사용
					...
					success : function(data){
						if(data.check){			//json사용했기때문에 data.으로 접근가능
							$("#id_check_msg").html("사용 가능한 이메일입니다.").css("color", "blue").attr("data-check","1");
						}else{
							$("#id_check_msg").html("중복된 이메일입니다.").css("color", "red").attr("data-check","0");
							$("#id").focus();
						}
					}
				});
