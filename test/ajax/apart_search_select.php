...
$sql="SELECT area_main_name, replace(area_sub_name,' ','') as area_sub_name, replace(dong,' ','') as dong, replace(apart_name,' ','') as apart_name, build_year, addr, doro, TRIM(LEADING '0' FROM doro_code1) as doro_code1 , TRIM(LEADING '0' FROM doro_code2) as doro_code2 FROM apart_dong where (apart_name like '%$q%' or addr like '%$q%' or concat(doro,' ', TRIM(LEADING '0' FROM doro_code1),' ',TRIM(LEADING '0' FROM doro_code2)) like '%$q%') and doro is not null limit 10";
