...
//여기는 그래프 관련
$sql_chart = "SELECT STR_TO_DATE(CONCAT(YEAR,LPAD(MONTH,'2','0'),LPAD(DAY,'2','0')), '%Y%m%d') AS dt, CAST(price AS DECIMAL(10,6)) AS temp  from molit_info_all  WHERE area_main_name = '$area_main_name'  and apart_name = '$apart_name'  and round(size,2) = '$size'  ORDER BY STR_TO_DATE(CONCAT(YEAR,LPAD(MONTH,'2','0'),LPAD(DAY,'2','0')), '%Y%m%d')";
        
$sql_min_max = "SELECT ROUND(count(1)/10) as cnt, ROUND(MAX(CAST(price AS DECIMAL(10,2)))+2) AS temp_max, ROUND(MIN(CAST(price AS DECIMAL(10,2)))-2) AS temp_min  from molit_info_all  WHERE area_main_name = '$area_main_name'  and apart_name = '$apart_name'  and round(size,2) = '$size'";
...
<span style="font-size:30px; vertical-align: middle;"><b>아파트 매매 상세조회</b></span>
</center>
<a style="font-size:15px; float:right;" href='./info.php'>(텔레그램 매일 알림받기)</a>
<br>
<span style="font-size:25px;"><b><?php echo $area_main_name; ?> <?php echo $dong; ?> <?php echo $apart_name; ?>(건축년도 : <?php echo $build_year; ?>년) <br>전용면적 : <?php if( $all_area=="Y") { echo "전체";} else {echo $size; echo "㎡";}?>, 실거래 리스트(2017년 이후)</b></span>
<h1>
...
<table>
    ...
    <tbody>
      ...
      <tr>
          <td style="font-size: 20px;"><b><?=$row['apart_name']?></b></td>
          <td style="font-size: 20px;"><b><?=$row['yearmonthday']?></b></td>
          <td style="font-size: 20px;"><b><?=$row['size']?>㎡</b></td>
          <td style="font-size: 20px;"><b><?=$row['stair']?>층</b></td>
          <td style="font-size: 20px;"><b><?=$row['price']?>억</b></td>
          <td style="font-size: 20px;"><b><?=$row['type']?></b></td>
      </tr>
      <?php } ?>
    </tbody>
</table>
