...
<meta property="og:title" content="오늘집값 - 아파트 매매 상세검색">
...
if ($orderby=="" or $orderby=="latest_date"){
  ...
  $orderbytest = "order by cast(meme.last_price as decimal(10,2)) desc";
}elseif ($orderby=="max_all"){
  $orderbytest = "order by cast(meme.max_price as decimal(10,2)) desc";
}elseif ($orderby=="name"){
...
if ( $main == '1'){
  $main_text = "AND cast(meme.size as decimal(10,2)) > '49.99'";
}elseif ($main == ''){
  $main = "1";
  $main_text = "AND cast(meme.size as decimal(10,2)) > '49.99'";
}else{
...
$sql = "
      ...
      from molit_max_min_all meme LEFT join molit_max_min_rent_all rent
...
<span style="font-size:30px; vertical-align: middle;"><b>아파트 매매 조회</b></span>
</center>
...
<a style="float:right;" href='./apart_home_rent.php?area_main_name=<?=$area_main_name?>&area_sub_name=<?=$area_sub_name?>&dong=<?=$dong?>&apart_name=<?=$apart_name?>&orderby=<?=$orderby?>&main=<?=$main?>'>전체 전/월세 조회하기</a>
</h1>
...
<br>
<span style="font-size:20px;"><?php if ($area_main_name!="지역선택") {echo $area_main_name;} ?> <?php if ($area_main_name!="지역선택") {echo $area_sub_name;} ?> <?php echo $dong; ?> 실거래 리스트(2017년 이후), 검색조건당 최대 300개 조회</span>
...
<table>
    ...
    <tr>
        ...
        <th style="font-size: 20px;"><b>최고가격<br>대비하락<br>(전세가율)</b></th>
        <th style="font-size: 20px;"><b>최근거래<br>(최근전세)</b></th>
