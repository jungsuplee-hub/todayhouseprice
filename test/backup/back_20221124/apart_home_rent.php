...
if ($orderby=="" or $orderby=="latest_date"){
  ...
  $orderbytest = "ROUND((CAST(max_price as DECIMAL(10,5)) - CAST(last_price as DECIMAL(10,5))),2) desc";
}
...
?>
<center><span style="font-size:30px;"><b>전국 아파트 전/월세 조회</b></span><a style="font-size:15px;" href='http://1.239.38.238:8880/info.php'>(텔레그램알림받기)</a></center>
<h1>
<a href="http://1.239.38.238:8880/apart_today.php?area_main_name=전체&insert_date=<?=$today?>&main=<?=$main?>">금일 신규거래 리스트 바로가기</a>
<a style="float:right;" href='http://1.239.38.238:8880/apart_home.php'>부동산 실거래 조회 홈페이지</a>
</h1><br><br>
...
<br>
<span style="font-size:30px;"><?php if ($area_main_name!="지역선택") {echo $area_main_name;} ?> <?php if ($area_main_name=="지역선택" or $area_sub_name=="세종특별자치시") { echo ""; } else {echo $area_sub_name;} ?> <?php echo $dong; ?> 전/월세 리스트(2017년 이후)</span>
...
<table>
    ...
    <tbody>
      ...
      <tr>
          <td style="font-size: 20px;"><a href='http://1.239.38.238:8880/apart_rent.php?area_main_name=<?=$row[area_main_name]?>&apart_name=<?=$row[apart_name]?>&size=<?=$row[size]?>&dong=<?=$row[dong]?>&all_area=N'><b><?=$row['apart_name']?></b><br><?=$row['area_name']?> <?=$row['dong']?></td>
          <td style="font-size: 20px;"><b><?=$row['size']?>㎡</b></td>
          <td style="background-color:rgba(255, 0, 0, 0.5); font-size: 20px;"><b><?=$row['price_last']?>억</b></td>
