...
?>
<center><span style="font-size:30px;"><b>전국 아파트 실거래가 조회</b></span><a style="font-size:15px;" href='http://1.239.38.238:8880/info.php'>(텔레그램알림받기)</a></center>
<h1><a href="http://1.239.38.238:8880/apart_today.php?area_main_name=전체&insert_date=<?=$today?>&main=<?=$main?>">금일 신규거래 리스트 바로가기</a><a style="float:right;" href='http://1.239.38.238:8880/apart_home_rent.php'>부동산 전/월세 조회 홈페이지</a></h1>
...
<br>
<span style="font-size:30px;"><?php if ($area_main_name!="지역선택") {echo $area_main_name;} ?> <?php if ($area_main_name!="지역선택") {echo $area_sub_name;} ?> <?php echo $dong; ?> 실거래 리스트(2017년 이후)</span>
...
<table>
    ...
    <tbody>
      ...
      <tr>
          <td style="font-size: 20px;"><a href='http://1.239.38.238:8880/apart.php?area_main_name=<?=$row[area_main_name]?>&apart_name=<?=$row[apart_name]?>&size=<?=$row[size]?>&dong=<?=$row[dong]?>&all_area=N'><b><?=$row['apart_name']?></b><br><?=$row['area_name']?> <?=$row['dong']?></td>
