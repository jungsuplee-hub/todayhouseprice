...
if($area_sub_name!=''){
  $sql = "
        ...
        from molit_max_min_all_officetel_group meme LEFT join molit_max_min_rent_all_officetel_group rent
...
<?php
$rs3_select = mysqli_query($Conn, "select replace(dong,' ','') as dong from officetel_dong where area_main_name = '$area_main_name' and replace(area_sub_name,' ','') = replace('$area_sub_name','$area_main_name','') group by area_main_name, area_sub_name, dong");
...
<span style="font-size:25px;"><b>주택명 : </b></span><select style="width:170px;font-size:25px;" name="apart_name" id="apart_name" onchange="apart_apart_list(this)">
<option value=''>선택</option>
<?php
$rs4_select = mysqli_query($Conn, "select replace(apart_name,' ','') as apart_name from officetel_dong where area_main_name = '$area_main_name' and replace(area_sub_name,' ','') = replace('$area_sub_name','$area_main_name','') and replace(dong,' ','') = '$dong' group by area_main_name, area_sub_name, dong, apart_name");
...
<br>
<span style="font-size:20px;"><?php if ($area_main_name!="지역선택") {echo $area_main_name;} ?> <?php if ($area_main_name!="지역선택") {echo $area_sub_name;} ?> <?php echo $dong; ?> 실거래 리스트, 검색조건당 최대 100개 조회</span>
...
<table>
    ...
    <tr>
    <?php if($isMobile == "Y") { ?>
        <th style="font-size: 20px; width:32%;"><b>주택명</b></th>
        <th style="font-size: 18px; width:15%;"><b>전용면적</b></th>
        <th style="font-size: 18px; width:19%;"><b>최고가격<br>대비하락<br>(전세가율)</b></th>
        <th style="font-size: 18px; width:17%;"><b>최근거래<br>(최근전세)</b></th>
        <th style="font-size: 18px; width:17%;"><b>최고가격</b></th>
    <?php }else { ?>
...
if($area_sub_name!=''){
  $sql_rent = "
        select
          ...
          IFNULL(ROUND(rent.last_price/meme.last_price*100),0) AS rent_rate
          from molit_max_min_rent_all_officetel_group rent LEFT join molit_max_min_all_officetel_group meme
...
<table>
    ...
    <tr>
    <?php if($isMobile == "Y") { ?>
        <th style="font-size: 20px; background: #809EAD;  width:32%;"><b>주택명</b></th>
        <th style="font-size: 18px; background: #809EAD;  width:15%;"><b>전용면적</b></th>
        ...
        <th style="font-size: 18px; background: #809EAD;  width:17%;"><b>최고가격</b></th>
    <?php }else { ?>
        <th style="font-size: 20px; background: #809EAD;  width:35%;"><b>주택명</b></th>
        <th style="font-size: 20px; background: #809EAD;  width:11%;"><b>전용면적</b></th>
    ...
    <tbody>
      ...
      <tr>
          ...
          <td style="background-color:rgba(255, 0, 0, 0.3); font-size: 20px;"><b><?=$row_rent['price_last']?>억<br><?=$row_rent['percent']?>%하락</b><br>(<?=$row_rent['rent_rate']?>%)</td>
