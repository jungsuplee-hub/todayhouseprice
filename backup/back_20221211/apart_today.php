...
$sql_favorite_rent = "
      ...
      where today.insert_date = '$insert_date'
      and today.month_price = '0'
      and (today.area_main_name, today.dong, today.apart_name) in (select area_main_name, dong, apart_name from molit_favorite where userid = '$userid')
      and today.status is not null
      ORDER BY ABS(ROUND((CAST(today.rent_price as DECIMAL(10,5)) - CAST(today.last_price as DECIMAL(10,5))),2)) desc
      limit 300;
      ";
...
<table>
    <thead>
    <tr>
        <th style="font-size: 20px; background: #FF0000; width:37%;"><b>거래일자</b><br>아파트명</b></th>
        <th style="font-size: 20px; background: #FF0000; width:12%;"><b>전용면적</b><br>층<br>거래유형</b></th>
        <th style="font-size: 20px; background: #FF0000; width:12%;"><b>가격<br>(최근)</b></th>
        <th style="font-size: 20px; background: #FF0000; width:13%;"><b>이전가격</b><br>(이전전세)<br>(이전매매)</th>
...
$rs_sub = mysqli_query($Conn, "
            ...
            FROM
            (
              select replace(replace(area_name, concat(area_main_name,' '),''),' ','') as area_sub_name from molit_today_update today where $area_main_name_text and insert_date = '$insert_date' $main_text and status is not null group by area_main_name, area_name
              UNION all
              select replace(replace(area_name, concat(area_main_name,' '),''),' ','') as area_sub_name from molit_today_update_rent today where $area_main_name_text and insert_date = '$insert_date' $main_text and status is not null group by area_main_name, area_name
            ) AS a
...
<table>
    <thead>
    <tr>
        <th style="font-size: 20px; width:37%;"><b>거래일자</b><br>아파트명</b></th>
        <th style="font-size: 20px; width:12%;"><b>전용면적</b><br>층<br>거래유형</b></th>
        <th style="font-size: 20px; width:12%;"><b>가격<br>(최근)</b></th>
        <th style="font-size: 20px; width:13%;"><b>이전가격</b><br>(이전전세)</th>
    ...
    <tbody>
      ...
      <tr>
          <td style="font-size: 20px; width:37%;"><a href='./apart.php?area_main_name=<?=$row[area_main_name]?>&apart_name=<?=$row[apart_name]?>&size=<?=$row[size]?>&dong=<?=$row[doing]?>&all_area=N'><b><?=$row['yearmonthday']?></b><br><b><?=$row['area_main_name']?> <?=$row['area_name']?> <?=$row['doing']?></b><br><b><?=$row['apart_name']?></b></td>
          <td style="font-size: 20px; width:12%;"><b><?=$row['size']?>㎡</b><br><b><?=$row['stair']?>층</b><br><b><?=$row['TYPE']?></b></td>
          <?php
...
<table>
    <thead>
    <tr>
        <th style="font-size: 20px; background: #809EAD; width:37%;"><b>거래일자</b><br>아파트명</b></th>
        <th style="font-size: 20px; background: #809EAD; width:12%;"><b>전용면적</b><br>층<br>거래유형</b></th>
        <th style="font-size: 20px; background: #809EAD; width:12%;"><b>전세<br>(보증금)</b></th>
        <th style="font-size: 20px; background: #809EAD; width:13%;"><b>이전가격</b><br>(이전매매)</th>
    ...
    <tbody>
      ...
      <tr>
          <td style="font-size: 20px; width:37%;"><a href='./apart_rent.php?area_main_name=<?=$row_rent[area_main_name]?>&apart_name=<?=$row_rent[apart_name]?>&size=<?=$row_rent[size]?>&dong=<?=$row_rent[dong]?>&all_area=N'><b><?=$row_rent['yearmonthday']?> <br><?=$row_rent['area_main_name']?></b><b><?=$row_rent['area_name']?> <?=$row_rent[dong]?></b><br><b><?=$row_rent['apart_name']?></b></td>
          <td style="font-size: 20px; width:12%;"><b><?=$row_rent['size']?>㎡</b><br><b><?=$row_rent['stair']?>층</b><br><b><?=$row_rent['TYPE']?></b></td>
          <?php
