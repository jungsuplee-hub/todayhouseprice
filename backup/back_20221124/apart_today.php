...
$sql_status = "
  SELECT
      (SELECT COUNT(1) from molit_today_update WHERE insert_Date = '$insert_date' AND $area_main_name_text $main_text) AS total,
      ...
      IFNULL((SELECT COUNT(1) from molit_today_update WHERE insert_Date = '$insert_date' AND $area_main_name_text $main_text AND STATUS ='신규' GROUP BY STATUS),0) AS new,
      (SELECT SUM(ROUND((CAST(price as DECIMAL(10,5)) - CAST(last_price as DECIMAL(10,5))),2))from molit_today_update WHERE insert_Date = '$insert_date' AND $area_main_name_text $main_text AND STATUS IN ('신고가','상승')) AS up_price,
      (SELECT SUM(ROUND((CAST(last_price as DECIMAL(10,5)) - CAST(price as DECIMAL(10,5))),2))from molit_today_update WHERE insert_Date = '$insert_date' AND $area_main_name_text $main_text AND STATUS IN ('신저가','하락')) AS down_price
      FROM DUAL;
";
...
?>
<center><span style="font-size:30px;"><b>전국 아파트 실거래가(전세) 조회</b></span><a style="font-size:15px;" href='http://1.239.38.238:8880/info.php'>(텔레그램알림받기)</a></center>
<center>(오전7시 업데이트)</center>
<h1><a href='http://1.239.38.238:8880/apart_home.php'>부동산 실거래 조회 홈페이지</a><a style="float:right;" href='http://1.239.38.238:8880/apart_home_rent.php'>부동산 전/월세 조회 홈페이지</a></h1>
<h1>
...
<?php echo $insert_date; ?> 신규 실거래 리스트 <br>
<span style="font-size:30px;"><b>50㎡이상: </b></span><select style="width:50px;font-size:22px;" name="main_size" id="main_size" onchange="apart_main(this)">
	<option value="1" <?php if($main=='1'){echo 'selected';}?>>Y</option>
	<option value="0" <?php if($main=='0'){echo 'selected';}?>>N</option>
</select>
<br>
<span style="font-size:25px;">총 <?php echo $row_status['total']; ?>건, 총 상승금액 : <?php echo $row_status['up_price']; ?>억, 총 하락금액 : <?php echo $row_status['down_price']; ?>억<br>(신고가 <?php echo $row_status['upup']; ?>건, 상승 <?php echo $row_status['up']; ?>건, 동일 <?php echo $row_status['same']; ?>건, 하락 <?php echo $row_status['down']; ?>건, 신저가 <?php echo $row_status['downdown']; ?>건, 신규 <?php echo $row_status['new']; ?>건)</span>

<script>
function apart_list(e) {
  <?php echo "window.location.replace('./apart_today.php?'+'area_main_name='+document.getElementById('main').value+'&insert_date=$insert_date'+'&main=$main');"?>
}
function apart_main(e) {
  <?php echo "window.location.replace('./apart_today.php?'+'area_main_name='+document.getElementById('main').value+'&insert_date=$insert_date'+'&main='+document.getElementById('main_size').value);"?>
}
</script>
...
<table>
    ...
    <tbody>
      ...
      <tr>
          <td style="font-size: 20px;"><a href='http://1.239.38.238:8880/apart.php?area_main_name=<?=$row[area_main_name]?>&apart_name=<?=$row[apart_name]?>&size=<?=$row[size]?>&dong=<?=$row[doing]?>&all_area=N'><b><?=$row['yearmonthday']?></b><br><b><?=$row['area_main_name']?></b><br><b><?=$row['area_name']?></b><br><b><?=$row['apart_name']?></b></td>
...
<table>
    ...
    <tbody>
      ...
      <tr>
          <td style="font-size: 20px;"><a href='http://1.239.38.238:8880/apart_rent.php?area_main_name=<?=$row_rent[area_main_name]?>&apart_name=<?=$row_rent[apart_name]?>&size=<?=$row_rent[size]?>&dong=<?=$row_rent[doing]?>&all_area=N'><b><?=$row_rent['yearmonthday']?></b><br><b><?=$row_rent['area_main_name']?></b><br><b><?=$row_rent['area_name']?></b><br><b><?=$row_rent['apart_name']?></b></td>
          <td style="font-size: 20px;"><b><?=$row_rent['size']?>㎡</b><br><b><?=$row_rent['stair']?>층</b><br><b><?=$row_rent['TYPE']?></b></td>
          <?php 

          if ( $row_rent['STATUS']== '신고가') {
            echo "<td style='background-color:rgba(0, 0, 255, 0.5); font-size: 20px;'><b>$row_rent[rent_price]억</b><br>$row_rent[price_last]억<br>신고가</td>";
          } elseif ( $row_rent['STATUS']== '상승') {
            echo "<td style='background-color:rgba(0, 255, 0, 0.5); font-size: 20px;'><b>$row_rent[rent_price]억</b><br>$row_rent[price_last]억<br>상승</td>";
          } elseif ( $row_rent['STATUS']== '동일') {
            echo "<td style='background-color:rgba(255, 255, 0, 0.5); font-size: 20px;'><b>$row_rent[rent_price]억</b><br>$row_rent[price_last]억<br>동일</td>";
          } elseif ( $row_rent['STATUS']== '하락') {
            echo "<td style='background-color:rgba(255, 165, 0, 0.5); font-size: 20px;'><b>$row_rent[rent_price]억</b><br>$row_rent[price_last]억<br>하락</td>";
          } elseif ( $row_rent['STATUS']== '신저가') {
            echo "<td style='background-color:rgba(255, 0, 0, 0.5); font-size: 20px;'><b>$row_rent[rent_price]억</b><br>$row_rent[price_last]억<br>신저가</td>";
          } else 
          {
