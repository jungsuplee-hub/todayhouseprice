...
$sql_chart = "
  select
  dt, case_BUY_A1000, case_BUY_11000, case_BUY_A2000, case_BUY_A2001, case_BUY_A3000, case_BUY_A9000
  from
  (
  select concat(substr(RESEARCH_DATE,1,4),'/',substr(RESEARCH_DATE,5,2),'/',substr(RESEARCH_DATE,7,2)) as dt,
...
<script type="text/javascript">
    ...
    function drawChart() {
        ...
        var header = ['Date&Time(MM-DD HH:MM)', '전국', '서울', '수도권', '지방권', '6대광역시', '9개도'];
        var row = "";
        var rows = new Array();
        jQuery.each(chart_array, function(index, item) {
            row = [
                item.dt,  // 너무 긴 날짜 및 시간을 짧게 추출
                Number(item.case_BUY_A1000),
                Number(item.case_BUY_11000),
                Number(item.case_BUY_A2000),
                Number(item.case_BUY_A2001),
                Number(item.case_BUY_A3000),
                Number(item.case_BUY_A9000)
            ];
            rows.push(row);
        });
